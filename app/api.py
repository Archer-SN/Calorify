...
async def analyze_food(food_name, is_importing=False):
    ...
    if parser_request.status_code == 200:
        food_objs = []
        data_list = None
        # Checking what type is the response
        ...
        for data in data_list:
            ...
            food_obj.save()
            # Any random micronutrient will do for checking whether we should call for extra nutrients info
            if FoodNutrient.objects.filter(nutrient_id="P").exists:
                print(food_obj)
...
async def analyze_meal_plan(food_dict_list, is_importing=False):
    food_obj_dict_list = []

    # This is used to create a coroutine so that we can execute all the loops in parallel
    async def one_iteration(i):
        print(i)
        food_dict = food_dict_list[i]
        food_objs = await analyze_food(
            food_dict["food_name"], is_importing=is_importing
        )
        if food_objs:
    ...
    # Coroutines
    coros = [one_iteration(i) for i in range(len(food_dict_list))]
...
async def import_user_meal_plan(user, gpt_response, date=datetime.now()):
    messages = [
        DEFAULT_SYSTEM_MESSAGE,
    ...
    if response_message.get("function_call"):
        ...
        if (
            ...
            function_response = await analyze_meal_plan(
            ...
            for food_obj_dict in function_response:
                user_food = await import_user_food(user, food_obj_dict, date)
                if not user_food:
